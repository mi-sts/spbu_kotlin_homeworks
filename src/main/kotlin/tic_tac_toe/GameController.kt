...
object GameController {
    ...
    object GameField {
        ...
        fun setCell(xPos: Int, yPos: Int, cellType: CellType): Boolean {
            if (!gameOver || !isPositionsExist(xPos, yPos) || getCell(xPos, yPos) != CellType.EMPTY) return false
        ...
        fun getRow(position: Int): List<CellType>? {
            if (!isPositionExist(position)) return null
    ...
    private fun checkWin(placedXPos: Int, placedYPos: Int) {
        ...
        if (isTie && isAllCellsMarked()) finishGame(CellType.EMPTY)
    }
